...
impl User {
    ...
    pub fn read(conn: &mut PgConnection, user_id: &str) -> QueryResult<User> {
        users::table.find(user_id).first(conn)
    }

    pub fn update(conn: &mut PgConnection, user_id: &str, user: User) -> QueryResult<User> {
        diesel::update(users::table.find(user_id)).set(user).get_result(conn)
    }

    pub fn delete(conn: &mut PgConnection, user_id: &str) -> QueryResult<usize> {
